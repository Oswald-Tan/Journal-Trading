...
const Header = ({
  ...
  const tabs = [
    ...
    { id: 'education', label: 'Edukasi', path: '/education' },
  ];
  ...
  if (subscriptionLoading && !propsSubscription) {
    return (
      <div className="sticky top-0 z-50">
        ...
        {/* Bottom Header Loading */}
        <header className="bg-white border-b-2 border-gray-100">
          <div className="max-w-11/12 mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-center items-center h-12">
              <div className="animate-pulse bg-gray-200 h-6 w-64 rounded-xl"></div>
            </div>
          </div>
        </header>
      </div>
  ...
  return (
    <div className="sticky top-0 z-50">
      {/* TOP HEADER - Logo, Balance, Upgrade, Profile */}
      <header className="bg-white border-b-2 border-gray-100">
        <div className="max-w-11/12 mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            {/* Logo dan Brand */}
            ...
            <nav className="flex space-x-8">
              {tabs.map(tab => (
                ...
                >
                  <Link
                    to={tab.path}
                    className={`py-2 text-base rounded-xl font-medium transition-all duration-200 flex items-center space-x-2 ${
                      activeTab === tab.id 
                        ? 'text-orange-400' 
      ...
      {/* Mobile Navigation Menu - PERBAIKAN: Tambahkan scroll */}
      <AnimatePresence>
        {mobileMenuOpen && (
          <Motion.nav 
            ...
            className="md:hidden bg-white/95 backdrop-blur-md border-b-2 border-orange-200 fixed left-0 right-0 z-40 overflow-y-auto"
            style={{ 
              top: '64px', // Sesuaikan dengan tinggi top header
              maxHeight: 'calc(100vh - 64px)', // Batasi tinggi maksimum
              bottom: 0 // Pastikan menu bisa di-scroll sampai bawah
            }}
          >
            <div className="px-4 py-3 space-y-2 pb-6"> {/* Tambahkan padding bottom */}
              {/* Navigation Tabs */}
              {tabs.map(tab => (
                <Motion.div
                  key={tab.id}
              ...
              <div className="pt-3 border-t-2 border-orange-200">
                ...
                <Motion.button
                  ...
                  className="w-full mt-3 mb-3 px-4 py-3 rounded-xl bg-emerald-500 text-white font-bold flex items-center justify-center space-x-2 border-2 border-emerald-400"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                ...
                {/* Mobile Logout Button - Sekarang lebih mudah diakses */}
                <Motion.button
                  whileHover={{ x: 5 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
